...
Ejecutar con: streamlit run supervisor.py
Pasar supervisor por query param: ?supervisor=Zerbatto Jose Luis
"""
...
.sup-card {
    ...
    border-left: 4px solid #e0e0e0; cursor: pointer;
    transition: box-shadow .15s;
}
.sup-card:hover { box-shadow: 0 3px 10px rgba(0,0,0,0.12); }
...
.sup-stats { font-size: 12px; color: #555; margin-top: 10px; }
.sup-stat-num { font-size: 20px; font-weight: 800; }
...
if not supervisor_sel:
    ...
    # Enriquecer con métricas por supervisor
    ...
    resumen = resumen.sort_values("score_max", ascending=False)
    ...
    for i, (_, row) in enumerate(resumen.iterrows()):
        ...
        alerta_txt = ""
        if criticos > 0:
            alerta_txt = f'<span style="color:#E24B4A;font-weight:700;">{criticos} crítico{"s" if criticos>1 else ""}</span>'
            if altos > 0:
                alerta_txt += f' · <span style="color:#E65100;">{altos} alto{"s" if altos>1 else ""}</span>'
        elif altos > 0:
            alerta_txt = f'<span style="color:#E65100;">{altos} alto{"s" if altos>1 else ""}</span>'
        else:
            alerta_txt = '<span style="color:#2E7D32;">Sin alertas activas</span>'
        ...
        with cols[i % 3]:
            ...
            <div class="sup-card {cc}">
              ...
              <div style="display:flex;gap:20px;margin-top:12px;">
                <div>
                  <div class="sup-stat-num">{int(row['activos'])}</div>
                  <div style="font-size:11px;color:#aaa;">activos</div>
                </div>
                <div>
                  <div class="sup-stat-num">{perm_str}</div>
                  <div style="font-size:11px;color:#aaa;">perm. prom.</div>
                </div>
                <div style="margin-left:auto;align-self:center;">
                  {_bdg(nivel)}
                </div>
              </div>
              <div style="font-size:12px;margin-top:10px;">{alerta_txt}</div>
            </div>
            """, unsafe_allow_html=True)

            if st.button(f"Ver mis vendedores →", key=f"btn_{row['supervisor']}",
...
if not onb.empty:
    ...
    <table class="ot">
    <thead><tr>
      <th>Vendedor</th><th>Tipo</th><th>Mes</th><th>% Plan</th><th>Riesgo</th>
    </tr></thead>
    <tbody>{ob_rows}</tbody>
    </table>
    </div>""", unsafe_allow_html=True)
